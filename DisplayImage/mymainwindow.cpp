...
void MyMainWindow::slotActionOpenBaseFile()
{
  ...
  pShowimagebaseUI2=new ShowImageBase2(0,pBaseFileDataset);
  
  connect(this,SIGNAL(ReportCloseWindows()),pShowimagebaseUI1,SLOT(close()));
  connect(this,SIGNAL(ReportCloseWindows()),pShowimagebaseUI2,SLOT(close()));
  connect(pShowimagebaseUI1,SIGNAL(ReportPosition(QPoint,int,int)),pShowimagebaseUI2,SLOT(MoveImage(QPoint,int,int)));
}
  ...
  if (treeBaseItem==treeWidget->currentItem())
    {
      ...
      pShowimagebaseUI2->GetRasterband(nRGBofBase);
    } 
  return true;
}
...
void MyMainWindow::GetBasePosition(QPoint p)
{
