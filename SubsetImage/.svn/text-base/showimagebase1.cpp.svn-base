...
bool ShowImageBase1::ShowOverviewGray()
{
  ...
  emit ReportPosition(QPoint(0,0),nRectWidth*nScaleFactorWidth,nRectHeight*nScaleFactorHeight);
  show();
  return true;
}
...
void ShowImageBase1::mouseMoveEvent(QMouseEvent *e)
{
  ...
  if(e->modifiers()==Qt::ControlModifier)
    {
      if(nRectWidth+n_dx<0)
	{
	  nRectWidth=10;
	  n_dx=0;
	}
      if(nRectWidth+n_dx>nWidth-nPointStartRect.x())
	{
	  nRectWidth=nWidth-nPointStartRect.x();
	  n_dx=0;
	}
      if(nRectHeight+n_dy<0)
	{
	  nRectHeight=10;
	  n_dy=0;
	}
      if(nRectHeight+n_dy>nHeight-nPointStartRect.y())
	{
	  nRectHeight=nHeight-nPointStartRect.y();
	  n_dy=0;
	}
      nRectWidth=nRectWidth+n_dx;
      nRectHeight=nRectHeight+n_dy;

      //nRectItem->setRect(nPointStartRect.x(),nPointStartRect.y(),nRectWidth,nRectHeight);
      scene->removeItem(nRectItem);
      nRectItem=scene->addRect(nPointStartRect.x(),nPointStartRect.y(),nRectWidth,nRectHeight,QPen(Qt::red));
    }
  else{
  ...
  emit ReportPosition(TransOvToSrcPos(nPointStartRect),nRectWidth*nScaleFactorWidth,nRectHeight*nScaleFactorHeight);
}
...
bool ShowImageBase1::CalRectWidth()
{
  ...
  nRectHeight=nShowImageWidth/nScaleFactorHeight;
	
  if (nRectWidth>p2->GetXSize())
    {
      nRectWidth=p2->GetXSize();
    }
  if (nRectHeight>p2->GetYSize())
    {
      nRectHeight=p2->GetYSize();
    }
  nRectItem=scene->addRect(0,0,nRectWidth,nRectHeight,QPen(Qt::red));
  return true;
}
